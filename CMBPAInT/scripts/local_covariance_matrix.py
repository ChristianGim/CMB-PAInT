def calculate_covariance_matrix_local(Cls, Nside, Lmax, Out_PATH, Pol, TPol, EB, TB):

    """This function computes the signal pixel-pixel covariance matrix.
       It allows parallelization with the MPI standard.
       This script is developed for local computation taking the advantage of the total number of available CPUs.
    ...
    if Pol == False:
        Cls_TT = Cls[0, :] if Cls.ndim > 1 else Cls
        Cov_T = np.zeros((len(Indices_U), Npix))
        ...
        for i in range(N_Rows):
            ...
            Cov_T[i, :(Indices_Pix[i]+1)] = covariance_tools.Scalar_MatCov(Cls_TT, Lmax, x, Npix = None)
    ...
    if Pol == False:
        newCov_T = comm.gather(Cov_T, root = 0)
        ...
        if rank == 0:
            New_Cov_T = np.vstack(newCov_T)

            if (os.path.exists(Out_PATH) == False):
                os.makedirs(Out_PATH)
                
            np.save(Out_PATH+"/Cov_TT_lmax_"+str(Lmax)+".npy", New_Cov_T)
            
            if (os.path.exists(Out_PATH+"/Time_Stats") == False):
                os.mkdir(Out_PATH+"/Time_Stats")
                
            with open(Out_PATH+'/Time_Stats/covmat_TT_timer.txt', 'w') as f:
                f.write(f'Execution time: {np.mean(execution_time):.2f} seconds')
                np.save(Out_PATH+"/Time_Stats/covmat_TT_timer", execution_time)

        comm.barrier()

        np.save(Out_PATH + "/Time_Stats/covmat_timer_T_rank_"+str(rank)+".npy", execution_time)
        ...
        if rank == 0:
            ...
            if TPol:
                with open(Out_PATH+'/Time_Stats/covmat_TPol_timer.txt', 'w') as f:
                    f.write(f'Execution time: {np.mean(execution_time):.2f} seconds')
                    np.save(Out_PATH+"/Time_Stats/covmat_TPol_timer", execution_time)
            else:
                with open(Out_PATH+'/Time_Stats/covmat_Pol_timer.txt', 'w') as f:
                    f.write(f'Execution time: {np.mean(execution_time):.2f} seconds')
                    np.save(Out_PATH+"/Time_Stats/covmat_Pol_timer", execution_time)

        comm.barrier()

        np.save(Out_PATH + "/Time_Stats/covmat_timer_Pol_rank_"+str(rank)+".npy", execution_time)
...
if __name__ == '__main__':
    ...
    calculate_covariance_matrix_local(Cls, Nside, Lmax, Out_PATH, Pol, TPol, EB, TB)
