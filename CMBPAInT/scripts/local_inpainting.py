def calculate_inpainting(Input_Path, Output_Path, Name, Chol_Path, I_Mask, P_Mask, Pol, TPol, Num_Sims, Single, No_Z, Cons_Uncons, Fields, Zbar_Path):
    ...
    if No_Z:
        ...
        if Single:
            ...
            if rank == 0:
                ...
                if (os.path.exists(os.path.join(Output_Path, "Z")) == False):
                    os.mkdir(os.path.join(Output_Path, "Z"))
    ...
    if rank == 0:
        ...
        if (os.path.exists(os.path.join(Output_Path, "Time_Stats")) == False):
            os.makedirs(os.path.join(Output_Path, "Time_Stats"))
    ...
    if Single:
        ...
        for i in range(len(Indices)):
            ...
            if TPol and Pol:
                ...
                Returns = inpainting_tools.Inpaint(Map_Path, R, Lbar, Z, z_bar = z_bar, T_Mask = None, Cons_Uncons = Cons_Uncons, Pol = True, Pol_Mask = P_Mask, TPol = False, Fields = Fields)
...
if __name__ == '__main__':
    ...
    from CMBPAInT.Tools import inpainting_tools
    ...
    calculate_inpainting(Input_Path, Output_Path, Name, Chol_Path, I_Mask, P_Mask, Pol, TPol, Num_Sims, Single, No_Z, Cons_Uncons, Fields, Zbar_Path)
